...
public class BookGenerator {
    ...
    public Book createBook() {
        ...
        if (rand < 25) {
            ...
            book.setBookData(arrayEnEdNames[random.current().nextInt(0, arrayEnEdNames.length)],
                    arrayEnEdAuthors[random.current().nextInt(0, arrayEnEdAuthors.length)],
                    arrayEnEdUniversities[random.current().nextInt(0, arrayEnEdUniversities.length)]);
            return book;
        } else if (rand < 50) {
            Book book = GetBookFactory.createBook(BookType.FICTION_EN);
            book.setBookData(arrayEnFicNames[random.current().nextInt(0, arrayEnFicNames.length)],
                    arrayEnFicAuthors[random.current().nextInt(0, arrayEnFicAuthors.length)]);
            return book;
        } else if (rand < 75) {
            Book book = GetBookFactory.createBook(BookType.EDUCATION_RU);
            book.setBookData(arrayRuEdNames[random.current().nextInt(0, arrayRuEdNames.length)],
                    arrayRuFicAuthors[random.current().nextInt(0, arrayRuFicAuthors.length)]);
            return book;
        } else {
            Book book = GetBookFactory.createBook(BookType.FICTION_RU);
            book.setBookData(arrayRuFicNames[random.current().nextInt(0, arrayRuFicNames.length)],
                    arrayRuFicAuthors[random.current().nextInt(0, arrayRuFicAuthors.length)]);
            return book;
        }
    }
